...
void printWifiStatus();void sendHttpResponse(WiFiClient client);
void setup() {
  Serial.begin(9600);
...
void loop() {
  ...
  if (client) {
    ...
    const unsigned long requestTimeout = 5000; // 5 segundos
    ...
    if (requestComplete) {
      ...
      // Esperar un poco más para asegurar que todos los datos se envíen
      delay(200);
      
      // Verificar si hay más datos para enviar
      client.flush();
    }
